...
const getAllPagesWithUser = async (accessToken: string): Promise<any> => {
  ...
  for (const response of getIGAccPromiseResults) {
    ...
    getIGAccInfoPromiseArray.push(getIGUserMetrics(igAcc.id, accessToken));
  }
...
const getAllAccounts = async (accessToken: string): Promise<any> => {
  try {
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadContent() {
      ...
      const arrayOfInstagramBusinessAccounts: Array<IGBusinessAccount> = await getAllPagesWithUser(
        accessToken
      );
      setLoaded(true);
